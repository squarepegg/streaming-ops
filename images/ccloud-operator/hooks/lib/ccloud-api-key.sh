...
function ccloud::api_key::build_api_key_secret_name() {
  ...
  local "${@}"
  local secret_name="cc.api-key.$service_account_name.$resource_id"
  secret_name=$(common::to_lower_case string_to_lower="$secret_name")
  echo "$secret_name"
}
...
function ccloud::api_key::apply() {
  ...
  local sa_id=$(ccloud::sa::get_id name=$service_account_name) || {
    echo "error retrieving service account id for $service_account_name"
  ...
  [[ -n "$existing_secret" ]] && {
    ...
    ccloud_api_key=$(echo "${ccloud_api_key}" | base64 -d)
  }
